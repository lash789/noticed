...
const db = await fetch(`${SUPABASE_URL}/rest/v1/moments`, {
  ...
  body: JSON.stringify({ text: hasText ? text : null, image_url: imageUrl })
});
...
container.innerHTML = ‘’;
shuffle(moments).forEach((m, i) => container.appendChild(buildBubble(m, i)));
initScrollReveal();
}
...
const bob      = BOBS[Math.floor(rng() * BOBS.length)];
const sizeClass = SIZES[Math.floor(rng() * SIZES.length)];
const dur      = (3.5 + rng() * 4).toFixed(1) + ‘s’;
const delay    = (rng() * 2).toFixed(1) + ‘s’;
...
// Inner shape — always fills wrapper 100%
